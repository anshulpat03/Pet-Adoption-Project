...
def create_pet(pet_data):
    ...
    cursor.execute(
        """
        INSERT INTO pets (name, breed, age, description, image)
        VALUES (?, ?, ?, ?, ?)
        """,
        (
            pet_data.get("name"),
            ...
            pet_data.get("age"),
            pet_data.get("description", "No description available"),
            pet_data.get("image", None)  # Default to None if no image provided
        )
    )
    conn.commit()
