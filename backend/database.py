...
def init_user_db():
    ...
    if conn is not None:
        try:
            cursor = conn.cursor()
            cursor.execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT NOT NULL, email TEXT NOT NULL UNIQUE, password TEXT NOT NULL)')
...
def init_pets_db():
    ...
    cursor.executemany('INSERT INTO pets (name, breed, age, description, image) VALUES (?, ?, ?, ?, ?)', sample_pets)
    conn.commit()
    conn.close()
...
def init_adoption_progress_db():
    ...
    if conn is not None:
        try:
            cursor = conn.cursor()
            cursor.execute('CREATE TABLE IF NOT EXISTS adoption_progress (id INTEGER PRIMARY KEY, user_id INTEGER, pet_id INTEGER, status TEXT NOT NULL, FOREIGN KEY(user_id) REFERENCES users(id), FOREIGN KEY(pet_id) REFERENCES pets(id))')
